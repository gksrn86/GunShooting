...
public class Enemy : LivingEntity {
    ...
    {
        AudioMananger.instance.PlaySound("Impact", transform.position);
        if(damage >= health)
        {
            if(damage >= health)
            {
            ...
            AudioMananger.instance.PlaySound("Enemy Death", transform.position);
        ...
        if (hasTarget)
        {
            if (Time.time > nextAttackTime)
            {
                ...
                if (sqrDstToTarget < Mathf.Pow(attackDistanceThreshold + myCollsionRadius + targetCollisionRadius, 2))
                {
                    ...
                    AudioMananger.instance.PlaySound("Enemy Attack", transform.position);
